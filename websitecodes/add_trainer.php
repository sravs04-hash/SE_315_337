...
if (isset($_REQUEST['trainer'])) {
  ...
  // Check if the pay_id exists in the payment table
  $pay_check_query = "SELECT * FROM payment WHERE pay_id='$pay_id' LIMIT 1";
  $pay_result = mysqli_query($conn, $pay_check_query);
  
  if (mysqli_num_rows($pay_result) == 0) {
    // If pay_id doesn't exist in the payment table, add an error
  ...
  // If there are no errors, insert the new trainer
  if (count($errors) == 0) {
